    ...
    <>
      ...
      <Section>
        <article className="mx-auto max-w-3xl">
          ...
          >
            <Pi name="pi-arrow-left" className="text-sm" /> Back to blogs
          </Link>
                ...
                {post.sections.map((s) => (
                  <button
                    ...
                    className={cn(
                      ...
                      activeTab === s.id ? "text-gold-dark" : "text-ink-soft hover:text-foreground",
                    )}
                  >
                    {s.label}
                    {activeTab === s.id && (
          ...
          <div className="mt-12 rounded-2xl border border-line bg-surface p-6 text-center">
            <h3 className="text-xl text-foreground">Ready to experience this?</h3>
            <p className="mt-2 text-sm text-ink-soft">
              We&apos;ll craft a private journey around the stories that moved you.
            </p>
            <Link
              href="/plan-your-journey"
              className="mt-4 inline-flex items-center gap-2 rounded bg-gold px-6 py-3 text-sm font-semibold text-neutral-900 hover:bg-gold-dark transition-colors"
            >
              Plan your journey
            </Link>
          </div>
      ...
      <div className="mt-5 grid gap-5 sm:grid-cols-2">
        {related.map((p) => (
          ...
          >
            ...
            <h4 className="mt-1.5 font-medium text-foreground group-hover:text-gold-dark transition-colors">
              {p.title}
            </h4>
