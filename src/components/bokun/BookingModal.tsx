...
export function BookingModal({
  ...
  const allImages = useMemo(() => photoUrls?.length ? photoUrls : ["/images/victoria-falls.jpeg"], [photoUrls]);
          ...
          <div className="mt-4 space-y-3">
            {cart.items.map((item) => (
              <div key={item.id} className="flex items-center gap-4 rounded-xl border border-line bg-muted p-4">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={item.imageUrl ?? "/images/victoria-falls.jpeg"} alt="" className="h-14 w-20 rounded-lg object-cover shrink-0" />
