...
import { ProductBookingPanel } from "@/components/bokun/ProductBookingPanel";
  ...
  return (
    <>
      ...
      <Section className="!pt-8">
        <Container>
          {/* Breadcrumb */}
            ...
            <Link href="/#featured-tours" className="hover:text-gold-dark transition-colors">
              Tours
            </Link>
            <Pi name="pi-chevron-right" className="text-[10px]" />
      ...
      <Section className="!py-6">
        <Container>
          <div className="flex items-center gap-2 text-sm text-ink-soft">
            ...
            <a
              href={`https://wa.me/${siteConfig.whatsappNumber.replace(/\D/g, "")}?text=${encodeURIComponent(shareText + " " + shareUrl)}`}
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-1 rounded-full border border-line px-3 py-1.5 hover:bg-muted hover:text-foreground transition-colors"
            >
      ...
      {relatedProducts.length > 0 && (
        <Section muted>
          <Container>
            <h2 className="text-2xl text-foreground">Similar tours</h2>
              ...
              {relatedProducts.map((rp) => (
                ...
                >
                  <div className="aspect-[16/11] overflow-hidden">
                    <Photo
                      src={rp.photoUrl ?? "/images/victoria-falls.jpeg"}
                      alt={rp.title}
